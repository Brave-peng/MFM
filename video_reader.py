...
#                     data = [x[0:11] for x in data]
    ...
    def get_seq(self, label, idx=-1, modality='rgb'):
        '''
            功能描述:
                根据输入的label和idx,找到对应的视频;根据modality,选择对应模态的最终视频地址。
            补充: 
                此处的视频,指的是包含8个jpg的文件夹, 实质是一个视频抽帧后的序列文件。

            返回:
                图片堆叠后的numpy格式, 和视频的id。
        '''
        c = self.get_train_or_test_db()
    ...
    # dance数据集专用，获取固定视角的getitem
    def get_fixed_view_rgb_seq(self, label, idx=-1, view='Camera_0', modality='rgb'):
        c = self.get_train_or_test_db()
        paths, vid_id = c.get_rand_vid(label, idx)
        if modality == 'S3D':
            s3d = np.load(os.path.join(paths[0]))
    ...
    def get_single_modality_task(self, index):
        ...
        def get_c_v(label, idx):
            c = self.get_train_or_test_db()
            paths, vid_id = c.get_rand_vid(label, idx)
            path = paths[0].split(os.sep)
            class_folders, video_folders = path[-3], path[-2]
            return class_folders, video_folders
        ...
        for bl, bc in enumerate(batch_classes):
            ...
            for idx in idxs[0:self.args.shot]:
                if self.args.fixed_view is not None:
                    vid, vid_id = self.get_fixed_view_rgb_seq(bc, idx, self.args.fixed_view, self.args.modality)
                ...
                support_c_v.append(get_c_v(bc, idx))
            ...
            for idx in idxs[self.args.shot:]:
                ...
                real_target_labels.append(bc)
                target_c_v.append(get_c_v(bc, idx))
        
        s = list(zip(support_set, support_labels, real_support_labels, support_vid, support_c_v))
        random.shuffle(s)
        support_set, support_labels, real_support_labels, support_vid, support_c_v = zip(*s)
        
        t = list(zip(target_set, target_labels, real_target_labels, target_vid, target_c_v))
        random.shuffle(t)
        target_set, target_labels, real_target_labels, target_vid, target_c_v = zip(*t)
    ...
    def get_random_modality_task(self, index):
        ...
        s = list(zip(support_set, support_labels, support_c_v))
        random.shuffle(s)
        support_set, support_labels, support_c_v = zip(*s)
        
        t = list(zip(target_set, target_labels, real_target_labels, target_c_v))
        random.shuffle(t)
        target_set, target_labels, real_target_labels, target_c_v = zip(*t)
