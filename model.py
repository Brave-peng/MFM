...
from utils import MyLoss, split_first_dim_linear
...
class TrxBranch(nn.Module):
    ...
    def forward(self, context_features, target_features, context_labels):
        ...
        support_videos = context_features
        # Frame-level logits
        all_logits_fr = [t(context_features, context_labels, target_features)[
            'logits'] for t in self.transformers]
        # 20 x 5 x 1[number of timesteps] 20 - 5 x 4[5-way x 4 queries/class]
        ...
        return {'logits': split_first_dim_linear(sample_logits_fr, [NUM_SAMPLES, target_features.shape[0]]), 
...
class STRMBranch(nn.Module):
    ...
    def forward(self, context_features, context_labels, target_features):
        ...
        return_dict = {'logits': split_first_dim_linear(sample_logits_fr, [NUM_SAMPLES, target_features.shape[0]]),
                       'support_videos': context_features_fr}
...
class ThreeTRXShuffleTime(nn.Module):
    '''
        一个三模态TRX融合的网络。
        其中shuffle的具体方法是利用torch.randperm()随机打乱模态数据的时间顺序实现的。
    '''
    def __init__(self, args):
        super().__init__()
        self.args = args
    ...
    def forward(self, context_features, context_labels, target_features):
        ...
        # 对tensor的第二维度进行data shuffle
        def shuffle_dim2(tensor):
            length = tensor.size(1)
            random_indices = torch.randperm(length)
            tensor_shuffle = tensor[:,random_indices,:]
            return tensor_shuffle
        
        second_context_shuffle = shuffle_dim2(second_context)
        second_target_shuffle = shuffle_dim2(second_target)
        third_context_shuffle = shuffle_dim2(third_context)
        third_target_shuffle = shuffle_dim2(third_target)
...
class ThreeTRXShiftTime1(nn.Module):
    '''
        一个三模态TRX融合的网络。
        其中shuffle的具体方法是利用torch.randperm()随机打乱模态数据的时间顺序实现的。
    '''
    def __init__(self, args):
        ...
        self.fusion2 = TwoTransforFusion(args)
        self.three_fusion = ThreeTransforTemproal(args)
        self.ids = range(args.num_gpus)
    ...
    def forward(self, context_features, context_labels, target_features):
        ...
        # 对tensor的第二维度进行移位，即将第二维度的数据向后移动一位
        def shift_dim2(tensor):
            length = tensor.size(1)
            tensor_shift = torch.cat((tensor[:,1:,:], tensor[:,0,:].unsqueeze(1)), dim=1)
            return tensor_shift
        
        second_context_shuffle = shift_dim2(second_context)
        second_target_shuffle = shift_dim2(second_target)
        third_context_shuffle = shift_dim2(third_context)
        third_target_shuffle = shift_dim2(third_target)

        fusion_context1 = self.three_fusion(first_context, second_context, third_context)
        fusion_target1 = self.three_fusion(first_target, second_target, third_target)

        fusion_context2, fusion_target2 = self.fusion1(
            first_context, first_target, second_context_shuffle, second_target_shuffle,
        )

        fusion_context3, fusion_target3 = self.fusion1(
            first_context, first_target, third_context_shuffle, third_target_shuffle
        )

        fusion_context4, fusion_target4 = self.fusion2(
            fusion_context2, fusion_target2, fusion_context3, fusion_target3
        )

        fusion_context = fusion_context1 + fusion_context4
        fusion_target = fusion_target1 + fusion_target4
        del fusion_context1, fusion_context2, fusion_target1, fusion_target2, fusion_context3, fusion_target3
...
class FiveTransforTemproal(nn.Module):
    def __init__(self, args, out_channels=None, dropout=0.1):
        super().__init__()
        in_channels = 2048
        self.positionEncoding1 = TrainablePositionalEncoding(
        ...
        self.positionEncoding4 = TrainablePositionalEncoding(
            args.seq_len, in_channels)
        self.positionEncoding5 = TrainablePositionalEncoding(
            args.seq_len, in_channels)
        encoder_layer = nn.TransformerEncoderLayer(
            d_model=in_channels * 5, nhead=1, batch_first=True)
        self.transformer_encoder = nn.TransformerEncoder(
            encoder_layer, num_layers=args.trans_num)
        self.f1 = nn.Linear(in_channels * 5, in_channels)
        self.dropout = nn.Dropout(dropout)

    def forward(self, x1, y1, z1, h1, q1):
        x1 = self.positionEncoding1(x1)
        y1 = self.positionEncoding2(y1)
        z1 = self.positionEncoding3(z1)
        h1 = self.positionEncoding4(h1)
        q1 = self.positionEncoding5(q1)
        xyzhq1 = torch.cat((torch.cat((torch.cat((torch.cat((x1, y1), dim=-1), z1), dim=-1), h1),dim=-1), q1), dim=-1)
        fusion1 = self.transformer_encoder(xyzhq1)
        return self.dropout(self.f1(fusion1))
...
class FiveShiftFusion(nn.Module):
    def __init__(self, args):
        ...
        self.bracnch = TrxBranch(args, 0 % args.num_gpus)
        self.fusion = TwoTransforFusion(args)
        self.five_fusion = FiveTransforTemproal(args)
        self.ids = range(args.num_gpus)
    ...
    def forward(self, context_features, context_labels, target_features):
        ...
        fifth_context_suffix = fifth_context[:, :self.args.shirt_num, :]
        fifth_context_shuffle = torch.cat((fifth_context_suffix, fifth_context_shuffle), dim=1)

        fifth_target_shuffle = fifth_target[:, self.args.shirt_num:, :]
        fifth_target_suffix = fifth_target[:, :self.args.shirt_num, :]
        fifth_target_shuffle = torch.cat((fifth_target_suffix, fifth_target_shuffle), dim=1)

        fusion_context1 = self.five_fusion(first_context, second_context, third_context, fourth_context, fifth_context)
        fusion_target1 = self.five_fusion(first_target, second_target, third_target, fourth_target, fifth_target)
...
class ThreeSTRMShiftLoopTime(nn.Module):
    def __init__(self, args):
        super().__init__()
        self.args = args
        self.bracnch = STRMBranch(args, 0 % args.num_gpus)
        self.fusion = TwoTransforFusion(args)
        self.three_fusion = ThreeTransforTemproal(args)
        self.ids = range(args.num_gpus)
...
class S3D(nn.Module):
    ...
    def forward(self, context_images, context_labels, target_images):
        ...
        print("context_images.shape", context_images.shape)
        context_features = self.encoder(context_images)  # 200 x 2048
        target_features = self.encoder(target_images)  # 160 x 2048
...
class ThreeAdd(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.args = args
        self.bracnch = TrxBranch(args, 0 % args.num_gpus)
        self.three_fusion = nn.Linear(args.trans_linear_in_dim, args.trans_linear_in_dim)
        self.ids = range(args.num_gpus)
    ...
    def forward(self, context_features, context_labels, target_features):
        ...
        fusion_context = first_context + second_context + third_context
        fusion_target = first_target + second_target + third_target
        fusion_context = self.three_fusion(fusion_context)
        fusion_target = self.three_fusion(fusion_target)
        support_videos = fusion_context
...
class ThreeCat(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.args = args
        self.bracnch = TrxBranch(args, 0 % args.num_gpus)
        self.three_fusion = nn.Linear(args.trans_linear_in_dim * 3, args.trans_linear_in_dim)
        self.ids = range(args.num_gpus)
    ...
    def forward(self, context_features, context_labels, target_features):
        ...
        fusion_context =  torch.cat((first_context, second_context, third_context), dim=-1)
        fusion_target = torch.cat((first_target, second_target, third_target), dim=-1)
        fusion_context = self.three_fusion(fusion_context)
        fusion_target = self.three_fusion(fusion_target)
        support_videos = fusion_context
