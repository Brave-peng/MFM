...
class AuxDataset(torch.utils.data.Dataset):
    def __init__(self, args):
        ...
        self.d = parsing_label(PurePath(self.args.traintestlist) / "trainlist03.txt")
...
#                     data = [x[0:11] for x in data]
    ...
    def get_seq(self, video_path, modality='rgb'):
        ...
        path_list[-3] = f'{modality}_Camera_0'
        path_list.insert(0, '/')
        path = PurePath(*path_list)
        paths = self.f(path)
        paths = sorted(paths)
    ...
    def get_video(self, index):
        video_path = self.train_test_lists[self.mode][index]
        return self.get_seq(video_path, self.args.modality)
