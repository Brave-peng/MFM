...
import my_utiles
from my_utiles.LossBuilder import LossBuilder
...
class Learner:
    ...
    def parse_command_line(self):
        ...
        parser.add_argument("--base_model", type=str, default="TRM")
        parser.add_argument("--shirt_num", type=int, default=1)
        parser.add_argument("--trans_num", type=int, default=2)
        ...
        if args.method in ["resnet50", "resnet34"]:
            args.img_size = 224

        args.trans_linear_in_dim = 2048 if args.method == "resnet50" else 512
        
        if args.dataset == "ssv2":
            args.traintestlist = os.path.join(
                args.scratch,
                "video_datasets/splits/ssv2TrainTestlist")
            args.path = os.path.join(args.scratch,
                                     "video_datasets/data/ssv2/rgb_l8")
    ...
    def train_task(self, task_dict):
        ...
        # flops, params = profile(self.model, inputs=(context_fea, context_labels, target_fea))
        # print("flops: ", flops, "params: ", params)
        # exit()
        model_dict = self.model(context_fea, context_labels, target_fea)
        target_logits = model_dict["logits"]
        support_videos = model_dict["support_videos"]
        ...
        task_loss = self.loss(target_logits, target_labels, device=self.device) 
        ...
        with torch.no_grad():
            ...
            for task_dict in self.video_loader:
                ...
                target_logits = self.model(context_fea, context_labels,
                                           target_fea)['logits'].to(self.device)
                target_labels = target_labels.to(self.device)
                accuracy = self.accuracy_fn(target_logits, target_labels)

                # # 将每个query的预测情况记录到lib/example中
                # """
                ...
                # with open('lib/hmdb_example.csv', 'a', newline='') as csvfile:
    ...
    # 用于可视化展示网站
    def demo(self, prefix="/media/disk1/zp/dataset/kinetics"):
        self.model.eval()
        with torch.no_grad():
            self.video_loader.dataset.train = False
            accuracy_dict = {}
            ...
            for task_dict in self.video_loader:
                ...
                # 从本地读取demo的视频地址，加载并使用pywebio进行渲染
                def web():
                    try:
                    ...
                    pywebio.output.put_table([['query'] + query_video,
                               ['predict'] + d['predict']['predict'],
                               ['real'] + d['predict']['real']]).style("margin-left: 50px")

                    out = [[
                        pywebio.output.span('action\support'), 'support1', 'support2',
                        'support3', 'support4', 'support5'
                    ]] + [[key] + dd[key] for key in dd]

                    with pywebio.output.use_scope("scope") as scope:
                        pywebio.output.put_table(out)
