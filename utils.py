...
class MyLoss(nn.Module):
    ...
    def forward(self, support_videos:list):
...
def parsing_label(path):
    data = pathlib.Path(path).read_text()
    l = data.strip().split("\n")
    l = [x.replace(' ', '_') for x in l]
    l = [x.strip().split(" ")[0] for x in l]
    class_set = defaultdict(int)
    for i in l:
        p = pathlib.PurePath(i)
        c = p.parts[0]
        class_set[c] += 1
    a = enumerate(class_set)
    return dict(map(lambda x: (x[1], x[0]), a))