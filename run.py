...
import my_utiles
from my_utiles import LossBuilder
import torch
import numpy as np
import argparse
import os
...
from model import TRX, ResnetBranch, TrxBranch
...
class Learner:
    def __init__(self):
        ...
        self.loss = LossBuilder.build_loss(self.args.loss)(args=self.args).loss
    ...
    def parse_command_line(self):
        ...
        parser.add_argument("--dataset", choices=["ssv2", "kinetics", "hmdb", "ucf", "dance"], default="ssv2", help="Dataset to use.")
        ...
        parser.add_argument("--cross_view", type=bool, default=False)
        parser.add_argument("--model", type=str, default='TRX')
        parser.add_argument("--view", type=int, default=3)
        parser.add_argument("--fixed_view", type=str, default=None)
        parser.add_argument("--loss", type=str, default='TRXLoss')
        parser.add_argument("--delete_dir", type=bool, default=False)
        parser.add_argument(
            "--fixed_test_eposide",
            type=str,
        ...
        parser.add_argument(
            "--modality", type=str, default="rgb", help="rgb or flow or skeleton or flow"
        )

        parser.add_argument("--getitem_name", type=str, default="get_single_modality_task")
        ...
        if args.dataset == "ssv2":
            args.traintestlist = os.path.join(
                args.scratch, "video_datasets/splits/ssv2TrainTestlist"
            )
            args.path = os.path.join(
                args.scratch,
                f"video_datasets/data/{args.dataset}/{args.modality}_l8",
            )
            ...
            args.path = os.path.join(
                args.scratch, f"video_datasets/data/{args.dataset}/{args.modality}_l8"
            )
        ...
        with tf.compat.v1.Session(config=config) as session:
                ...
                for task_dict in self.video_loader:
                    ...
                    if (iteration + 1) % self.args.print_freq == 0:
                        ...
                        t = time.gmtime()
                        print_and_log(self.logfile,time.strftime("%Y-%m-%d %H:%M:%S",t) + ' Task [{}/{}], Train Loss: {:.7f}, Train Accuracy: {:.7f}'
    ...
    def train_task(self, task_dict):
        ...
        flops, params = profile(self.model, inputs=(context_images, context_labels, target_images))
        print("flops: ", flops, "params: ", params)
        exit()

        model_dict = self.model(context_images, context_labels, target_images)
        target_logits = model_dict['logits'].to(self.device)
        # my_loss = model_dict['my_loss'].to(self.device)
        # print(my_loss)
    ...
    def test(self, session, num_episode):
        ...
        with torch.no_grad():
                ...
                for task_dict in self.video_loader:
                    ...
                    context_images, target_images, context_labels, target_labels, real_target_labels, batch_class_list = self.prepare_task(task_dict)
                    flops, params = profile(self.model.cuda(), inputs=(context_images.cuda(), context_labels.cuda(), target_images.cuda()))
                    print("flops: ", flops, "params: ", params)
                    exit()
                    if self.args.getitem_name != "get_multi_feature_task":
                        model_dict = self.model(context_images, context_labels, target_images)
                    else:
                        model_dict = self.model.test(context_images, context_labels, target_images)
                    target_logits = model_dict['logits'].to(self.device)

                    # 将每个query的预测情况记录到lib/{modality}example中
                    """
                    为什么不把这个写入到checkpoint_dir中？
                    因为每次进行测试的时候都会调用这个代码。如果放到checkpoint_dir中，其实输出的是所有测试的预测情况。
                    而我们每次训练，测试大概都要测6-15次，所以记录的数据其实没有可参考性。
                    """
                    d = {}
                    ...
                    query_summery = list(zip(predict, real_class))
                    
                    with open(f'lib/{self.args.modality}_result.csv', 'a', newline='') as csvfile:
                        writer = csv.writer(csvfile)
                        for i in query_summery:
                            writer.writerow(i)
                    target_labels = torch.from_numpy(np.array(target_labels)).to(self.device)
